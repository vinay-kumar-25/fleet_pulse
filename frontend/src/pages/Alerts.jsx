...
export default function Alerts() {
  ...
  if (loading) return <div className={`p-8 text-center text-sm ${activeTheme.textSecondary}`}>Checking overdue service queues...</div>;
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      {alerts.length === 0 ? (
        <div className={`mac-card p-8 text-center space-y-2 ${activeTheme.card}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {alerts.map((item) => (
            <div key={item._id} className={`mac-card flex items-start justify-between p-5 ${activeTheme.danger}`}>
              <div className="space-y-1">
                <div className="text-base font-bold">{item.vehicle_id?.registration_number}</div>
                <div className={`text-xs ${activeTheme.textSecondary}`}>{item.vehicle_id?.make} {item.vehicle_id?.model}</div>
                <div className="text-sm font-medium mt-2">{item.description || 'Routine Interval Service'}</div>
              </div>

              <button
                onClick={() => handleDismiss(item._id)}
                className="mac-button bg-red-500 px-4 py-2 text-xs font-semibold text-white hover:bg-red-600"
              >
                Dismiss Alert
              </button>
            </div>
